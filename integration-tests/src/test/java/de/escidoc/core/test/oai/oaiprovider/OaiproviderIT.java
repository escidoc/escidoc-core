 ...
 */
public class OaiproviderIT extends OaiproviderTestBase {
    ...
    private void prepare() throws Exception {
        ...
        for (int i = 0; i < numItemsMusicType; i++) {
            itemIdsMusic[i] =
                getObjid(create(TEMPLATE_OAIPROVIDERTEST_ITEM_PATH + "/", "escidoc_oai_item0.xml", "music", null,
                    "music"));
        }
        for (int i = 0; i < numItemsVideoType; i++) {
            itemIdsVideo[i] =
                getObjid(create(TEMPLATE_OAIPROVIDERTEST_ITEM_PATH + "/", "escidoc_oai_item0.xml", "video", null,
                    "video"));
        }
        for (int i = 0; i < numItemsNoOai; i++) {
            itemIdsNoOai[i] =
                getObjid(create(TEMPLATE_OAIPROVIDERTEST_ITEM_PATH + "/", "escidoc_oai_item0.xml", "nooai", "nooai",
                    "nooai"));
        }
        ...
        for (int i = 0; i < numContainersMusicType; i++) {
            containerIdsMusic[i] =
                getObjid(create(TEMPLATE_OAIPROVIDERTEST_CONTAINER_PATH + "/", "escidoc_oai_container0.xml", "music",
                    null, "music"));
        }
        for (int i = 0; i < numContainersVideoType; i++) {
            containerIdsVideo[i] =
                getObjid(create(TEMPLATE_OAIPROVIDERTEST_CONTAINER_PATH + "/", "escidoc_oai_container0.xml", "video",
                    null, "video"));
        }
        for (int i = 0; i < numContainersNoOai; i++) {
            containerIdsNoOai[i] =
                getObjid(create(TEMPLATE_OAIPROVIDERTEST_CONTAINER_PATH + "/", "escidoc_oai_container0.xml", "nooai",
                    "nooai", "nooai"));
        }
