...
public class ContentModelTestBase extends CmmTestBase {
    ...
    public String getContentModelTitle(Document contentModel) throws Exception {
        return selectSingleNode(contentModel,
            "/content-model/properties/name/text()").getNodeValue();
    }

    public String getContentModelDescription(Document contentModel)
        throws Exception {
        return selectSingleNode(contentModel,
            "/content-model/properties/description/text()").getNodeValue();
    }

    public Map<String, String> getContentModelMdRecordDefinitions(
        Document contentModel) throws Exception {
            ...
            String xsd =
                selectSingleNode(
                    ...
                    "/content-model/md-record-definitions/"
                        + "md-record-definition[@name = '" + name
    ...
    public List<String> getContentModelResourceDefinitions(Document contentModel)
        throws Exception {
    ...
    public List<List<String>> getContentModelContentStreamDefinitions(
        Document contentModel) throws Exception {
     ...
     *             If anything fails.
     */
    public String retrieveContentModels(final Map<String, String[]> filter)
        throws Exception {

        return handleXmlResult(getContentModelClient().retrieveContentModels(
            filter));
    }

    public String retrieveContentStreams(final String id) throws Exception {
        return handleXmlResult(getContentModelClient().retrieveContentStreams(
            id));
    }

    public String retrieveContentStream(final String id, final String name)
        throws Exception {
        return handleXmlResult(getContentModelClient().retrieveContentStream(
            id, name));
    }
    ...
    public String retrieveVersionHistory(final String id) throws Exception {
        ...
        return handleXmlResult(getContentModelClient().retrieveVersionHistory(
            id));
    }
    ...
    protected void validateContentModel(
        String createdXML, String title, String description,
        Map<String, String> mdRecordDefinitions,
        List<String> resourceDefinitions,
        List<List<String>> contentStreamDefinitions, boolean checkBehavior)
        throws Exception {
        ...
        validateContentModelProperties(createdXML, id,
            "/content-model/properties", title, description,
        ...
        while (it.hasNext()) {
            ...
            assertEquals("Metadata definition has not expected Schema Href.",
                "/cmm/content-model/" + id
                    + "/md-record-definitions/md-record-definition/"
                    + mdRecordName + "/schema/content", selectSingleNode(
                    contentModel,
                    "/content-model/md-record-definitions/"
                        + "md-record-definition[@name = '" + mdRecordName
                        + "']/schema/@href").getNodeValue());
        ...
        while (it.hasNext()) {
            ...
            selectSingleNodeAsserted(contentModel,
                "/content-model/resource-definitions/resource-definition[@name = '"
                    + methodName + "']/xslt[@href = '" + "/cmm/content-model/"
                    + id + "/resource-definitions/resource-definition/"
                    + methodName + "/xslt/content']");
        }
        ...
        while (lit.hasNext()) {
            ...
            selectSingleNodeAsserted(contentModel,
                "/content-model/content-streams/content-stream[@name = '"
                    + contentStreamName + "']");
            assertEquals("Content Stream definition has not expected href.",
                "/cmm/content-model/" + id + "/content-streams/content-stream/"
                    + contentStreamName + "/content", selectSingleNode(
                    contentModel,
                    "/content-model/content-streams/content-stream[@name = '"
                        + contentStreamName + "']/@href").getNodeValue());
            assertEquals(
                "Content Stream definition has not expected mime-type.",
                contentStreamDefinition.get(1), selectSingleNode(
                    contentModel,
                    "/content-model/content-streams/content-stream[@name = '"
                        + contentStreamName + "']/@mime-type").getNodeValue());
            assertEquals(
                "Content Stream definition has not expected storage type.",
        ...
        if (checkBehavior) {
            ...
            // if(contentModel.isItemContentModel)
            {
                ...
                itemXml =
                    handleXmlResult(omBase.getItemClient().create(itemXml));
                Document itemResources = null;
                if (getTransport() == Constants.TRANSPORT_REST) {
                    String itemResourcesXml =
                        handleXmlResult(omBase
                            .getItemClient().retrieveResources(
                ...
                while (it.hasNext()) {
                    String methodName = it.next();
                    if (getTransport() == Constants.TRANSPORT_REST) {
                        selectSingleNodeAsserted(itemResources, "/resources/"
                            + methodName);
                    }
                }
            ...
            // else if(contentModel.isContainerContentModel)
            {
                ...
                OmTestBase omBase = new OmTestBase(getTransport());
                containerXml =
                    handleXmlResult(omBase.getContainerClient().create(
                        containerXml));
                Document containerResources = null;
                if (getTransport() == Constants.TRANSPORT_REST) {
                    String containerResourcesXml =
                        handleXmlResult(omBase
                            .getContainerClient().retrieveResources(
                ...
                while (it.hasNext()) {
                    String methodName = it.next();
                    if (getTransport() == Constants.TRANSPORT_REST) {
                        selectSingleNodeAsserted(containerResources,
    ...
    protected void validateContentModelProperties(
        String createdXML, String id, String propertiesXpath, String title,
        String description, String lastModificationDate) throws Exception {
        ...
        assertEquals("Name of content model not as expected.", title,
            selectSingleNode(contentModel, propertiesXpath + "/name/text()")
                .getNodeValue());
        assertEquals("Description of content model not as expected.",
            description, selectSingleNode(contentModel,
        ...
        String latestVersionNumber =
            selectSingleNodeAsserted(contentModel,
                propertiesXpath + "/latest-version/number/text()")
                .getNodeValue();
        String versionNumber =
            selectSingleNodeAsserted(contentModel,
                propertiesXpath + "/version/number/text()").getNodeValue();
        assertTrue(
            "Version number of latest version must be greater or equal to version number of current version.",
            (Integer.parseInt(latestVersionNumber) >= Integer
        ...
        if (getTransport() == Constants.TRANSPORT_REST) {
            ...
            selectSingleNodeAsserted(contentModel, propertiesXpath
                + "[@href = '/cmm/content-model/" + id + "/properties']");
            selectSingleNodeAsserted(contentModel, propertiesXpath + "/version"
                + "[@href = '/cmm/content-model/" + id + ":" + versionNumber
                + "']");
            selectSingleNodeAsserted(contentModel, propertiesXpath
                + "/latest-version" + "[@href = '/cmm/content-model/" + id
                + ":" + latestVersionNumber + "']");
            ...
            selectSingleNodeAsserted(contentModel, propertiesXpath + "/version"
                + "[@objid = '" + id + ":" + versionNumber + "']");
            selectSingleNodeAsserted(contentModel, propertiesXpath
                + "/latest-version" + "[@objid = '" + id + ":"
                + latestVersionNumber + "']");

            selectSingleNodeAsserted(contentModel, propertiesXpath
                + "/created-by/@objid");
            selectSingleNodeAsserted(contentModel, propertiesXpath
                + "/version/modified-by/@objid");
        }
        validateObjidForm(contentModel, propertiesXpath + "/version/modified-by");

        if (lastModificationDate != null) {
            // timestamps
            assertEquals(lastModificationDate, selectSingleNodeAsserted(
                contentModel, propertiesXpath + "/latest-version/date/text()")
                .getNodeValue());
            if (latestVersionNumber.equals(versionNumber)) {
                assertEquals(lastModificationDate, selectSingleNodeAsserted(
                    contentModel, propertiesXpath + "/version/date/text()")
                    .getNodeValue());
            }
            else {
                assertDateBeforeAfter(selectSingleNodeAsserted(contentModel,
     ...
     *             If anything fails.
     */
    protected void validateObjidForm(Document document, String elementPath)
        throws Exception {
        String refObjid = null;
        if (getTransport() == Constants.TRANSPORT_REST) {
            String href =
                selectSingleNodeAsserted(document, elementPath + "/@href")
                    .getNodeValue();
            refObjid = href.substring(href.lastIndexOf('/'));
        }
        else {
            refObjid =
                selectSingleNodeAsserted(document, elementPath + "/@objid")
                    .getNodeValue();
        }
