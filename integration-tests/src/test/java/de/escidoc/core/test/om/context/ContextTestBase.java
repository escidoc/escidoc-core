...
public class ContextTestBase extends OmTestBase {
     ...
     */
    protected String createSuccessfully(final String templateName)
        throws Exception {
     ...
     * 
     * @param xmlCreatedContext
     *            The created context.
     * @param xmlTemplateContext
     *            The template context used to create the context.
     * @param timestampBeforeCreation
    ...
    public void assertCreatedContext(
        final String xmlCreatedContext, final String xmlTemplateContext,
        final String timestampBeforeCreation) throws Exception {

        assertContext(xmlCreatedContext, xmlTemplateContext,
            timestampBeforeCreation, timestampBeforeCreation);
    }
     ...
     * 
     * @param toBeAssertedXml
     *            The context xml data that shall be asserted.
     * @param xmlTemplateContext
     *            The template context used to create the context.
     * @param timestampBeforeCreation
     *            A timestamp before the creation of the context.
    ...
    public void assertContext(
        ...
        assertRootElement(msg + "Root element failed. ", toBeAssertedDocument,
            XPATH_CONTEXT, Constants.CONTEXT_BASE_URI,
        ...
        assertPropertiesElementUnversioned("Properties failed. ",
            toBeAssertedDocument, XPATH_CONTEXT_PROPERTIES,
            timestampBeforeCreation);

        // assert properties
        assertXmlEquals("Context error: name was changed!", template,
            toBeAssertedDocument, XPATH_CONTEXT_PROPERTIES_NAME);
        ...
        NodeList expectedDescription =
            selectNodeList(template, XPATH_CONTEXT_PROPERTIES_DESCRIPTION);
        NodeList toBeAssertedDescription =
        ...
        if (expectedDescription.getLength() > 0) {
            String expDescription =
                expectedDescription.item(0).getTextContent();
            if ((expDescription != null) && (!expDescription.equals(""))) {
                assertXmlEquals(message, template, toBeAssertedDocument,
                    XPATH_CONTEXT_PROPERTIES_DESCRIPTION);
            }
            else if (toBeAssertedDescription.getLength() > 0) {
                String toAssertDescription =
                    toBeAssertedDescription.item(0).getTextContent();
        ...
        NodeList expectedOus =
            selectNodeList(template,
                XPATH_CONTEXT_PROPERTIES_ORGANIZATIONAL_UNIT);
        NodeList toBeAssertedOus =
            selectNodeList(toBeAssertedDocument,
                XPATH_CONTEXT_PROPERTIES_ORGANIZATIONAL_UNIT);
        int length = expectedOus.getLength();
        assertEquals(msg + "Number of OUs different. ", length, toBeAssertedOus
            .getLength());
        for (int i = 0; i < length; i++) {
            final String ouXpath =
                XPATH_CONTEXT_PROPERTIES_ORGANIZATIONAL_UNIT + "[" + (i + 1)
                    + "]";
            assertReferencingGroupElements(msg + "Asserting " + (i + 1)
                + ". OU. ", template, toBeAssertedDocument, ouXpath,
                XPATH_CONTEXT_PROPERTIES_ORGANIZATIONAL_UNIT,
     ...
     *             If anything fails.
     */
    protected String retrieveMembers(final String id, final Map<String, String[]> filter)
        throws Exception {

        return handleXmlResult(getContextClient().retrieveMembers(
            (String) checkParameter(id), filter));
    }
    ...
    public String retrieveAdminDescriptor(final String id, final String name)
        ...
        return handleXmlResult(getContextClient().retrieveAdminDescriptor(
            (String) checkParameter(id), (String) checkParameter(name)));
    }
    ...
    public String retrieveProperties(final String id) throws Exception {
        ...
        return handleXmlResult(getContextClient().retrieveProperties(
            (String) checkParameter(id)));
    }
     ...
     *            The id of the context.
     * @param taskParam
     *            The task parameters including the last-modification-date.
     * @return The result XML (result.xsd)
     * 
     * @throws Exception
     *             If anything fails.
     */
    protected String open(final String id, final String taskParam)
        throws Exception {
     ...
     *             If anything fails.
     */
    protected String close(final String id, final String taskParam)
        throws Exception {
