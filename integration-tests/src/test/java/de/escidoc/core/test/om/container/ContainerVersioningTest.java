...
public class ContainerVersioningTest extends ContainerTestBase {
    ...
    public void testRetrieveVersionHistory() throws Exception {
        ...
        assertHrefBase(versionHistoryDoc, "/version-history/version/@href",
            Constants.CONTAINER_BASE_URI + "/" + theContainerId + ":1");
    ...
    public void testIncrementVersionNumber() throws Exception {
        ...
        assertXmlExists("New version number", theContainerXml,
            "/container/properties/version/number[text() = '" + rounds + "']");
        ...
        for (int i = rounds; i > 0; i--) {
            ...
            assertXmlExists("Current-version number missmatch. (" + i + ")",
                xml, "/container/properties/version/number[text() = '" + i
                    + "']");

            if (i > 1) {
                assertXmlExists("Version " + i
                    + " content not as expected (positiv).", xml,
                    "/container/properties/content-model-specific/nox["
                        + (i - 1) + "]");
            }
            else {
                assertXmlNotExists("Version " + i
                    + " content not as expected (negativ).",
                    EscidocRestSoapTestsBase.getDocument(xml),
                    "/container/properties/content-model-specific/nox[" + i
                        + "]");
            }
    ...
    public void testLifecycle() throws Exception {
        ...
        assertXmlExists("Properties status submitted", xml,
            "/container/properties/public-status[text() = 'submitted']");
        ...
        assignObjectPid(theContainerId, getPidParam(theContainerId,
            "http://localhost:8080/ir/container/" + theContainerId));
        String containerId = theContainerId + ":1";
        assignVersionPid(containerId, getPidParam(containerId,
            "http://localhost:8080/ir/container/" + containerId));
        ...
        assertXmlExists("version status not released", xml,
        ...
        assertXmlExists("version status must be released if withdrawn", xml,
            "/container/properties/version/status[text() = 'released']");
        assertXmlExists("Further released withdrawn container latest-release",
            xml, "/container/properties/latest-release");
        assertXmlValidContainer(xml);
    ...
    public void testLifecycleVersions() throws Exception {
        ...
        Document versionHistoryDoc =
            getDocument(retrieveVersionHistory(theContainerId));
        ...
        assertEquals("eventIdentifierVaule " + " of Container ["
            + theContainerId + "] differs from defined format", XPathAPI
            .selectSingleNode(
                versionHistoryDoc,
                "/version-history/version[version-number='1']"
                    + "/events/event[1]/eventIdentifier/eventIdentifierValue")
            .getTextContent(), "/ir/container/"
            + theContainerId
        ...
        // check href of version-history/version element
        assertHrefBase(versionHistoryDoc, "/version-history/version/@href",
            Constants.CONTAINER_BASE_URI + "/" + theContainerId + ":2");
        // check objid of version-history/version element
        assertHrefObjidConsistency((Document) versionHistoryDoc,
            "/version-history/version");
        ...
        assertXmlExists("New version number", xml,
            "/container/properties/version/number[text() = '3']");
        assertXmlValidContainer(xml);

        versionHistoryDoc = getDocument(retrieveVersionHistory(theContainerId));
        // check href of version-history/version element
        assertHrefBase(versionHistoryDoc, "/version-history/version/@href",
            Constants.CONTAINER_BASE_URI + "/" + theContainerId + ":3");
        // check objid of version-history/version element
        assertHrefObjidConsistency((Document) versionHistoryDoc,
            "/version-history/version");
        ...
        xml = update(theContainerId, xml);
        assertXmlExists("New version number", xml,
            "/container/properties/version/number[text() = '4']");
        assertXmlExists("Properties status released", xml,
            "/container/properties/public-status[text() = 'released']");
        assertXmlExists("version status pending", xml,
            "/container/properties/version/status[text() = 'pending']");
        ...
        // check href of version-history/version element
        assertHrefBase(versionHistoryDoc, "/version-history/version/@href",
            Constants.CONTAINER_BASE_URI + "/" + theContainerId + ":4");
        // check objid of version-history/version element
        assertHrefObjidConsistency((Document) versionHistoryDoc,
            "/version-history/version");
        ...
        xml = update(theContainerId, xml);
        assertXmlExists("New version number", xml,
            "/container/properties/version/number[text() = '5']");
        assertXmlExists("Properties status released", xml,
            "/container/properties/public-status[text() = 'released']");
        assertXmlExists("version status submitted", xml,
            "/container/properties/version/status[text() = 'submitted']");
        ...
        // check href of version-history/version element
        assertHrefBase(versionHistoryDoc, "/version-history/version/@href",
            Constants.CONTAINER_BASE_URI + "/" + theContainerId + ":5");
        // check objid of version-history/version element
        assertHrefObjidConsistency((Document) versionHistoryDoc,
            "/version-history/version");
        ...
        assertXmlExists("Properties status withdrawn", xml,
            "/container/properties/public-status[text() = 'withdrawn']");
        assertXmlExists("version status withdrawn", xml,
            "/container/properties/version/status[text() = 'released']");
        assertXmlExists("Further released withdrawn container latest-release",
            xml, "/container/properties/latest-release");
        assertXmlValidContainer(xml);
        ...
        for (int versionNumber = 1; versionNumber < 6; versionNumber++) {
            assertEquals(
                "eventIdentifierValue " + " of Container [" + theContainerId
                    + "] differs from defined format",
                "/ir/container/"
                    + theContainerId
                    + "/resources/version-history#"
                    + XPathAPI
                        .selectSingleNode(
                            versionHistoryDoc,
                            "/version-history/version[version-number='"
                                + versionNumber + "']"
                                + "/events/event[1]/@xmlID").getTextContent(),
                XPathAPI
                    .selectSingleNode(
    ...
    /**
     * Test if a released Container get the right version-status after adding
     * further Items to it. The version-status has to change from release to
     * pending.
     * 
     * Proceeding: 1.) create container 2.) release container 3.) add item to
     * the container 4.) check if the container has new version-status (pending)
     * 5.) retrieve as anonymous user and count number of items (the new items
     * are not in the released version) 6.) release the latest version of the
     * Container 7.) retrieve and count the number of items (with user login)
     * 8.) retrieve and count the number of items (as anonymous)
     * 
     * See also Bug #5xx
     * 
     * @throws Exception
    ...
    public void testContainerStatusAfterAddingItems() throws Exception {
        ...
        NodeList itemList =
            selectNodeList(containerDoc, "/container/struct-map/item");
        ...
        assertEquals("Version status not changed ", STATE_RELEASED,
            getVersionStatus(containerDoc));
        ...
        Document tempItemDoc =
            EscidocRestSoapTestsBase.getDocument(tempItemXml);
        ...
        assertEquals("Increased Version number", contLtstVrsNo + 1,
            newltstVrsNo);

        // check status of latest version --------------------------------------
        assertEquals("Version status not changed ", STATE_PENDING,
            getVersionStatus(containerDoc));
     ...
     *            Set true if withdrawn comment is to include.
    ...
    public void testStatusLifecycleVersions() throws Exception {
        ...
        assertXmlExists(PUBLIC_STATUS_COMMENT, xml, propertiesBaseXPath
            + PUBLIC_STATUS_COMMENT);
        assertXmlExists(VERSION_COMMMENT, xml, propertiesBaseXPath
            + VERSION_COMMMENT);
        assertXmlExists("Properties status pending", xml, propertiesBaseXPath
            + "public-status[text() = 'pending']");
        assertXmlExists("version status pending", xml, propertiesBaseXPath
            + "version/status[text() = 'pending']");

        publicStatusComments.add(currentVersion, selectSingleNode(xmlDoc,
            propertiesBaseXPath + PUBLIC_STATUS_COMMENT + "/text()")
            .getNodeValue());
        versionComments.add(currentVersion, selectSingleNode(xmlDoc,
            propertiesBaseXPath + VERSION_COMMMENT + "/text()").getNodeValue());
        assertXmlValidContainer(xml);

        Document versionHistoryDoc;
        versionHistoryDoc =
            getDocument(retrieveVersionHistory(this.theContainerId));
        // check comment in version-history
        assertEquals(versionComments.get(currentVersion), selectSingleNode(
            versionHistoryDoc,
            "/version-history/version[@href = '" + Constants.CONTAINER_BASE_URI
                + "/" + this.theContainerId + ":" + currentVersion
                + "']/comment/text()").getNodeValue());
        assertEquals(publicStatusComments.get(currentVersion),
            selectSingleNode(
                versionHistoryDoc,
                "/version-history/version[@href = '"
                    + Constants.CONTAINER_BASE_URI + "/" + this.theContainerId
                    + ":" + currentVersion + "']/comment/text()")
                .getNodeValue());
        ...
        assertXmlExists("New version number", xml, propertiesBaseXPath
            + "version/number[text() = '2']");
        currentVersion = 2;
        assertXmlValidContainer(xml);
        ...
        assertXmlExists(VERSION_COMMMENT, xml, propertiesBaseXPath
            + VERSION_COMMMENT);
        assertXmlExists("Properties status submitted", xml, propertiesBaseXPath
            + "public-status[text() = 'submitted']");
        assertXmlExists("version status submitted", xml, propertiesBaseXPath
            + "version/status[text() = 'submitted']");
        ...
        assignObjectPid(this.theContainerId, getPidParam(this.theContainerId,
            "http://localhost:8080/ir/container/" + this.theContainerId));

        String versionId = this.theContainerId + ":3";
        assignVersionPid(versionId, getPidParam(versionId,
            "http://localhost:8080/ir/container/" + versionId));
        ...
        assertXmlExists(VERSION_COMMMENT, xml, propertiesBaseXPath
            + VERSION_COMMMENT);
        assertXmlExists("Properties status released", xml, propertiesBaseXPath
            + "public-status[text() = 'released']");
        assertXmlExists("version status released", xml, propertiesBaseXPath
        ...
        assertXmlExists(VERSION_COMMMENT, xml, propertiesBaseXPath
            + VERSION_COMMMENT);
        assertXmlExists("Properties status withdrawn", xml, propertiesBaseXPath
            + "public-status[text() = 'withdrawn']");
        assertXmlExists("version status must still be released", xml,
            propertiesBaseXPath + "version/status[text() = 'released']");
    ...
    public void testTimestamps01() throws Exception {
        ...
        assertEquals("last-modification-date in root attribute of Container ["
            + this.theContainerId + "] differs from //version/date", XPathAPI
            .selectSingleNode(xmlDoc, "/container/@last-modification-date")
            .getTextContent(), XPathAPI.selectSingleNode(xmlDoc,
            "/container/properties/version/date").getTextContent());

        // /container/@last-modification-date ==
        // /container/properties/latest-version/date
        assertEquals("last-modification-date in root attribute of Container ["
            + this.theContainerId + "] differs from //latest-version/date",
            XPathAPI.selectSingleNode(xmlDoc,
                "/container/@last-modification-date").getTextContent(),
            XPathAPI.selectSingleNode(xmlDoc,
        ...
        assertEquals("eventDateTime of the latest event of version 1 differs "
            + "from timestamp attribute of version 1 [" + this.theContainerId
            + "]", XPathAPI.selectSingleNode(
            versionHistoryDoc,
            "/version-history/version[version-number='1']"
        ...
        assertEquals("last-modification-date in root attribute of Container ["
            + this.theContainerId + "] differs from last-modification-date "
            + "attribute of version-history", XPathAPI.selectSingleNode(xmlDoc,
            "/container/@last-modification-date").getTextContent(), XPathAPI
            .selectSingleNode(versionHistoryDoc,
        ...
        assertEquals("last-modification-date in root attribute of Container ["
            + this.theContainerId + "] differs from creation date of version",
            XPathAPI
                .selectSingleNode(versionHistoryDoc,
                    "/version-history/version[version-number='1']/@timestamp")
                .getTextContent(), XPathAPI.selectSingleNode(xmlDoc,
                "/container/properties/creation-date").getTextContent());

        // /version-history/version[version-number='1']/timestamp ==
        // /container/@last-modification-date
        assertEquals("last-modification-date in root attribute of Container ["
            + this.theContainerId + "] differs from timestamp of version 1 "
        ...
        // == submitted
        assertEquals("eventType for release is not as expected", "submitted",
            XPathAPI.selectSingleNode(
                versionHistoryDoc,
                "/version-history/version[version-number='4']"
        ...
        assertEquals("eventType for release is not as expected", "released",
            XPathAPI.selectSingleNode(
                versionHistoryDoc,
                "/version-history/version[version-number='5']"
                    + "/events/event[1]/eventType").getTextContent());
        ...
        // /container/@last-modification-date
        assertEquals("eventDateTime of the latest event of version 1 differs "
            + "from last-modification-date root attribute of Container ["
            + this.theContainerId + "]", XPathAPI.selectSingleNode(
            versionHistoryDoc,
            "/version-history/version[version-number='5']"
