...
public class StatisticReaderTest extends GrantTestBase {
    ...
    private static String[] newReportDefinitionIds = 
                    new String[NUM_REPORT_DEFINITIONS];
    ...
    private static final String REPORT_PARAMETERS_TEMPLATE_NAME = 
                                            "escidoc_report_parameters0.xml";
    ...
    private void prepare() throws Exception {
        // create new scope.
        newScopeId = doTestCreateScope(PWCallback.DEFAULT_HANDLE,
                SCOPE_TEMPLATE_NAME, null);

        // create aggregationDefinition in new scope
        newAggregationDefinitionId = doTestCreateAggregationDefinition(
                PWCallback.DEFAULT_HANDLE,
                AGGREGATION_DEFINITION_TEMPLATE_NAME, newScopeId, null);

        // create reportDefinitions in new scope
        for (int i = 0; i < NUM_REPORT_DEFINITIONS; i++) {
            newReportDefinitionIds[i] = doTestCreateReportDefinition(
                    PWCallback.DEFAULT_HANDLE, REPORT_DEFINITION_TEMPLATE_NAME
                            .replaceAll("\\$\\{COUNTER\\}", "" + (i + 1)),
                    newScopeId, newAggregationDefinitionId, null);
        }

        // create statistic_data-records as system-admin in new scope
        for (int i = 0; i < NUM_STATISTIC_DATA; i++) {
            doTestCreateStatisticData(PWCallback.DEFAULT_HANDLE,
                    STATISTIC_DATA_TEMPLATE_NAME, newScopeId, null);
        }

        // create aggregationDefinition in scope 3
        aggregationDefinitionId = doTestCreateAggregationDefinition(
                PWCallback.DEFAULT_HANDLE,
                AGGREGATION_DEFINITION_TEMPLATE_NAME, SCOPE_ID, null);

        // create reportDefinitions in scope 3
        for (int i = 0; i < NUM_REPORT_DEFINITIONS; i++) {
            reportDefinitionIds[i] = doTestCreateReportDefinition(
                    PWCallback.DEFAULT_HANDLE, REPORT_DEFINITION_TEMPLATE_NAME
                            .replaceAll("\\$\\{COUNTER\\}", "" + (i + 1)),
                    SCOPE_ID, aggregationDefinitionId, null);
        }

        // create statistic_data-records as in scope 3
        for (int i = 0; i < NUM_STATISTIC_DATA; i++) {
            doTestCreateStatisticData(PWCallback.DEFAULT_HANDLE,
                    STATISTIC_DATA_TEMPLATE_NAME, SCOPE_ID, null);
        }
    ...
    private void triggerPreprocessing(
        ...
        String urlParameters =
            PREPROCESSING_URL + System.currentTimeMillis();
    ...
    /**
     * Tests successfully retrieving a report by an statistics_editor.
     * Report-definition may get executed by statistics-editor and depositor
     * 
     * @throws Exception
     *             If anything fails.
     */
    ...
    public void testRetrieveReportAllowed1() throws Exception {
        ...
        doTestRetrieveReport(HANDLE,
                REPORT_PARAMETERS_TEMPLATE_NAME, newReportDefinitionIds[1],
                null);
    }

    /**
     * Tests successfully retrieving a report by an default-user.
     * Report-definition may get executed by default-user and depositor
     * 
     * @throws Exception
     *             If anything fails.
     */
    ...
    public void testRetrieveReportAllowed2() throws Exception {
        ...
        doTestRetrieveReport(HANDLE,
                REPORT_PARAMETERS_TEMPLATE_NAME, newReportDefinitionIds[0],
                null);
    }

    /**
     * Tests successfully retrieving a report by an system-administrator.
     * Report-definition may get executed by no roles.
     * 
     * @throws Exception
     *             If anything fails.
     */
    ...
    public void testRetrieveReportAllowed3() throws Exception {
        ...
        doTestRetrieveReport(PWCallback.DEFAULT_HANDLE,
                REPORT_PARAMETERS_TEMPLATE_NAME, newReportDefinitionIds[2],
                null);
    }

    /**
     * Tests successfully retrieving a report by an statistic-reader.
     * Report-definition may get executed by no roles.
     * 
     * @throws Exception
     *             If anything fails.
    ...
    public void testRetrieveReportAllowed4() throws Exception {
        ...
        doTestRetrieveReport(HANDLE,
                REPORT_PARAMETERS_TEMPLATE_NAME, reportDefinitionIds[2], null);
    }
