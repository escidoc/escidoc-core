...
public interface ItemHandlerInterface {
     ...
     * 
     * @param filter
     *            map of key/value pairs containing the filter definition. See functional specification.
     ...
     * The Item has to be in public-status "pending" or "in-revision" and must be unlocked, otherwise the removing of
     * the Item will fail.<br/>
     * <b>Tasks:</b><br/>
     * <ul>
     * <li>All content relations pointing from and to that Item will be deleted.</li>
     ...
     *             Thrown if the authorization fails.
     * @throws InvalidStatusException
     *             Thrown if the Items status is not valid for executing the action.
     * @throws MissingMethodParameterException
     *             If a mandatory element value is missing.
     * @throws SystemException
     *             If an error occurs.
     ...
     * <li>The Component is created and all Component specific properties are added.</li>
     * <li>A new Version of the Item is created.</li>
     * <li>If the version-status of the Item is "released" the version-status of the new version is set to "pending".</li>
     * <li>The XML representation of the Component corresponding to XML schema is returned as output.</li>
     * </ul>
     * 
     * @param id
     *            The id of the Item. The Component will be added to this Item.
     * @param xmlData
     *            The XML representation of the Component to be added corresponding to XML-schema "components.xsd".
     ...
     * Only Components of Items in public-status "pending" or "submitted" can be deleted.<br/>
     * <b>Tasks:</b><br/>
     * <ul>
     * <li>The Item is accessed using the provided reference.</li>
     ...
     * <li>The XML data for that Component is delivered (no binary content is included, instead a link to the
     * subresource "content" is added.</li>
     * </ul>
     * 
     * @param id
     *            The id of the Item.
     ...
     * <li>The XML data is validated against the XML schema of an Item.</li>
     * <li>Optimistic Locking criteria is checked.</li>
     * <li>The technical Metadata Record is completed with additional attributes.</li>
     ...
     * <p/>
     * <li>A new Version of the Item is created.</li>
     * <li>In case of status "released" of the Item, the status of the new version is set to "in revision"</li>
     * <li>Timestamp of the latest modification of the Item in the system is updated to the current time.</li>
     * <li>The XML data is created with version-date, version and all other attributes which have been created or
     * modified by the framework for that Component. Link to binary data stream is changed according the new location.</li>
     ...
     * @param xmlData
     *            The XML representation of the Component to be updated corresponding to XML schema "components.xsd".
     ...
     * <li>The XML data for that list of Components is created (no binary data stream is added, instead a link to the
     * subresource "content" is added.</li>
     * <li>The XML representation of the subresource Components of the Item corresponding to XML-schema is returned as
     * output.</li>
     * </ul>
     * 
     * @param id
     ...
     *             Thrown if the Item does not have the specified metadata record.
     * @throws AuthenticationException
     *             Thrown if the authentication fails.
     * @throws AuthorizationException
     *             Thrown if the authorization fails.
     ...
     *            The id of the Item.
     * @param contentId
     *            The id of the Component the binary content should be retrieved from.
     * @return The binary content.
     * @throws ItemNotFoundException
     *             Thrown if an Item with the specified id cannot be found.
     * @throws ComponentNotFoundException
     *             Thrown if the Component containing the content cannot be found.
     * @throws AuthenticationException
     *             Thrown if the authentication fails.
     * @throws AuthorizationException
     *             Thrown if the authorization fails.
     ...
     * The transformer of the binary content is an additional service. This subresource provides access to the binary
     * content of an Item.<br/>
     * <b>Prerequisites:</b><br/>
     * The Item must exist<br/>
     * The Component must exist<br/>
     * The content must exist<br/>
     ...
     * <li>The Component is accessed using the provided ID.</li>
     * <li>The binary content is retrieved from the Component.</li>
     * <li>The binary content is transformed through the transformation service (digilib).<br/>
     ...
     * <li>The Item is accessed using the provided ID.</li>
     * <li>The Component is accessed using the provided ID.</li>
     * <li>The request is forwarded to the content service. Additional parameter are added to the forward, if the
     * service requires.</li>
     * </ul>
     * <br/>
     ...
     *            The id of the Item.
     * @param xmlData
     *            The XML representation of the metadata record of the Item corresponding to XML schema
     *            "md-records.xsd".
     ...
     * <li>The Item is accessed using the provided Id.</li>
     * <li>Differences between modifiable elements in the delivered XML data and the XML representation of the currently
     * stored metadata record are taken to modify the metadata record in the system.</li> *
     * <li>If the metadata record is modified a new version of the Item is created.</li>
     * <li>If the status of the latest version is "released" a new version is created and gets the status "pending"
     * otherwise a new version is created with the same version status as before. This also applies to the public-status
     * till it is once set to "released".</li>
     * <li>The XML input data is updated and some new data is added (see Chapter 4)</li>
     * <li>The XML representation of the metadata record corresponding to XML schema is returned as output.</li>
     * </ul>
     * 
     * @param id
     *            The id of the Item.
     ...
     * @param xmlData
     *            The XML representation of the metadata record to be updated corresponding to XML-schema
     *            "md-records.xsd".
     ...
     *             Thrown if an Item with the specified id cannot be found.
     * @throws ContentStreamNotFoundException
     *             Thrown if a content stream with the specified name cannot be found.
     * @throws AuthenticationException
     *             Thrown if the authentication fails.
     * @throws AuthorizationException
     *             Thrown if the authorization fails.
    ...
    /**
     * Retrieve a list with references to all Containers to that this Item is directly subordinated.<br />
     * <br />
     * <b>Prerequisites:</b><br />
     * <br />
     * The Item must exist.<br />
     ...
     * <li>The Item is accessed using the provided Id.</li>
     * <li>Optimistic Locking criteria is checked.</li>
     * <li>The latest version status of the Item is changed to "released". This also applies to public-status till it is
     ...
     * <li>The Item is accessed using the provided Id.</li>
     * <li>Optimistic Locking criteria is checked.</li>
     * <li>The status of the latest-version is changed to "submitted". This also applies to public-status till it is
     * once set to "released".</li>
     ...
     * <li>No new version is created.</li>
     * <li>Return XML structure complies to result.xsd.</li>
     * </ul>
     * <p/>
     * <b>Parameter for request:</b>
     * 
     * <pre>
     * &lt;param last-modification-date=&quot;1967-08-13T12:00:00.000+01:00&quot; /&gt;
     * </pre>
     ...
     * The used PID Service and the assignment behavior is configured in <b>escidoc-core[.custom].properties</b>.
     * <p/>
     * <b>Prerequisites:</b>
     * <p/>
     * The Item must exist<br/>
     * The Item is not locked.<br/>
     * No version PID is assigned to this version of the Item.
     * <p/>
     * <b>Tasks:</b><br/>
     * <ul>
     * <li>The Item is accessed using the provided Id including the version Identifier.</li>
     * <li>Optimistic Locking criteria is checked.</li>
     ...
     * <li>The assigned Persistent Identifier is returned within an XML structure complies to result.xsd.</li>
     * </ul>
     * <p/>
     * <b>Parameter for request:</b> (example)<br/>
     * <p/>
     ...
     * @param taskParam
     *            The timestamp of the last modification of the Item and at minimum the URL which is to register in the
     *            PID resolver. This parameter list is forwarded to the PID resolver and could be extended. (see example
     *            above)
     ...
     * The PID Service is configured in <b>escidoc-core[.custom].properties</b>.
     * <p/>
     * <b>Prerequisites:</b><br/>
     * <p/>
     * The Item must exist<br/>
     ...
     * <li>The Item is accessed using the provided Id.</li>
     * <li>Optimistic Locking criteria is checked.</li>
     * <li>The new relations, which have the Item as a source are created according to a provided data</li>
     * <li>Latest version date is updated</li>
     ...
     * @param taskParameter
     *            xml structure with a last modification date of the source Item and a list of sections containing
     ...
     * <li>The Item is accessed using the provided Id.</li>
     * <li>Optimistic Locking criteria is checked.</li>
     * <li>The method checks if a source Item contains all relations with provided data. Otherwise a
     ...
     * @param param
     *            xml structure with a last modification date of the source resource and a list of sections containing
